...
async function main() {
  ...
  // Add overview
  markdown += '## Overview\n\n';
  markdown += `This documentation covers the Core modules of the OPNsense API. There are ${
    coreModules.length
  } core modules with a total of ${coreModules.reduce((sum, m) => sum + m.endpoints.length, 0)} endpoints.\n\n`;
  ...
  markdown += '|--------|-------|\n';
  markdown += `| Total Core Modules | ${coreModules.length} |\n`;
  markdown += `| Total Endpoints | ${coreModules.reduce((sum, m) => sum + m.endpoints.length, 0)} |\n`;
  markdown += `| Modules with XML Models | ${coreModules.filter(m => m.modelUrl).length} |\n`;
  markdown += '\n';

  // Write to file
  const outputPath = path.join(outputDir, 'CORE.md');
